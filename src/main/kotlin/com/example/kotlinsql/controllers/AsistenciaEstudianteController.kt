    ...
    @ApiResponses(
        value = [
            ApiResponse(
                ...
                content = [Content(mediaType = "text/plain", examples = [ExampleObject(value = "Asistencia registrada para el estudiante")])]
            ),
            ApiResponse(
                responseCode = "400",
                description = "Datos inválidos",
                content = [Content(mediaType = "text/plain", examples = [ExampleObject(value = "Datos inválidos para registrar la asistencia")])]
            )
        ]
    )
    @PostMapping
    fun crear(@Valid @RequestBody request: AsistenciaEstudianteCreateRequest): String {
        val res = service.crear(request)
        return if (res > 0) "Asistencia registrada para el estudiante" else "Datos inválidos para registrar la asistencia"
    }
    ...
    @ApiResponses(
        value = [
            ApiResponse(
                ...
                content = [Content(mediaType = "text/plain", examples = [ExampleObject(value = "Relación asistencia-estudiante eliminada")])]
            ),
            ApiResponse(
                responseCode = "404",
                description = "Relación no encontrada",
                content = [Content(mediaType = "text/plain", examples = [ExampleObject(value = "Relación no encontrada")])]
            )
        ]
    )
